...
暗号化キーは既定では毎回入力が必要ですが、アンロック画面で「このブラウザに保存する」を選べば
IndexedDB(非extractable CryptoKey)に保存され、次回以降は自動的に利用できます。

**PWA対応**: ホーム画面への追加・アプリとしてのインストールに対応しています(`vite-plugin-pwa`)。
アプリ本体(JS/CSS/HTML/アイコン)はオフラインでも起動でき、既にログイン済みでメモがキャッシュ済み
(IndexedDB)であれば、オフラインでも一覧・閲覧ができます。新規登録・ログイン・データの更新には
サーバーとの通信が必要です。
