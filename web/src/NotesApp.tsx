...
export default function NotesApp({
  ...
  useEffect(() => {
    if (!selected) return;
    if (debouncedDraft.title === selected.title && debouncedDraft.body === selected.body) return;
    setSaveState('saving');
    void save(selected.uid, debouncedDraft.title, debouncedDraft.body, selected.createdAt).then(
      () => setSaveState('saved')
    );
  ...
  return (
    <div className="flex h-full">
      {/* 左: メモ一覧 */}
      <aside className="flex w-72 shrink-0 flex-col border-r border-white/5 bg-white/[0.02]">
        ...
        <nav className="thin-scrollbar flex-1 overflow-y-auto px-2 pb-3">
          {loading && (
            <p className="px-3 py-6 text-center text-xs text-slate-500">読み込み中…</p>
          )}
          {error && (
          ...
          {!loading && visibleNotes.length === 0 && (
            <p className="px-3 py-6 text-center text-xs text-slate-500">メモがありません</p>
          )}
                    ...
                    {tags.length > 0 && (
                      <div className="mt-1 flex flex-wrap gap-1">
                        {tags.slice(0, 3).map((t) => (
                          <span key={t} className="rounded bg-white/5 px-1 py-px text-[9px] text-slate-500">
                            #{t}
                          </span>
                        ))}
                      </div>
